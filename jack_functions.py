...
def gettoc(toc_prog):
    ...
    if jack_helpers.helpers[toc_prog].has_key('toc_cmd'):
        cmd = string.replace(jack_helpers.helpers[
...
# by setting the pregap attribute.
            ...
            actual_track.pregap = jack_CDTime.CDTime(
...
def check_genre_txt(genre):
    ...
    import jack_version
    error("illegal genre. Try '" + jack_version.prog_name +
...
def check_file(num, i, ext):
    ...
    if os.path.exists(i[NAME] + ext):
        ...
        s = jack_freedb.filenames[num].encode(
