...
def find_workdir():
    ...
    while (not os.path.exists(cf['_toc_file'])) or cf['_multi_mode']:
        ...
        if tries > 2:
            break
        if cf['_guess_mp3s']:
            jack_ripstuff.all_tracks = jack_functions.guesstoc(
                cf['_guess_mp3s'])
        else:
            if cf['_multi_mode']:
                ...
                if cf['_image_toc_file']:
                    ...
                    # cwd soon
                    cf['_image_toc_file'] = os.path.abspath(
                        cf['_image_toc_file'])
                    jack_ripstuff.all_tracks, dummy, dummy = jack_functions.cdrdao_gettoc(
                        cf['_image_toc_file'])
                else:
                    if cf['_image_file']:
                        warning(
                            "No TOC file for image '%s' specified, reading TOC from CD device." % cf['_image_file'])
                        cf['_image_file'] = os.path.abspath(cf['_image_file'])
                    jack_ripstuff.all_tracks = jack_functions.gettoc(
                        cf['_toc_prog'])
                    toc_just_read = 1
            ...
            for i in dirs:
                if os.path.exists(os.path.join(i, cf['_toc_file'])):
                    jack_dirs.append(i)
                    file_toc, dummy, dummy = jack_functions.cdrdao_gettoc(
            ...
            if cf['_multi_mode']:
                ...
                for i in unique_dirs:
                    jack_ripstuff.all_tracks, dummy, track1_offset = jack_functions.cdrdao_gettoc(
                        os.path.join(i, cf['_toc_file']))
                    err, jack_tag.track_names, jack_tag.locale_names, cd_id, revision = freedb_names(jack_freedb.freedb_id(
                        jack_ripstuff.all_tracks), jack_ripstuff.all_tracks, jack_ripstuff.all_tracks, os.path.join(i, cf['_freedb_form_file']), verb=0, warn=0)
            ...
            if len(unique_dirs) > 1:
                ...
                if cf['_create_dirs']:
                    ...
                    dir_name = jack_version.prog_name + "-" + \
                        jack_freedb.freedb_id(jack_ripstuff.all_tracks, warn=0)
                    if not os.path.exists(dir_name) and not os.path.isdir(dir_name):
                        os.mkdir(dir_name)
                    os.chdir(dir_name)
                    jack_freedb.dir_created = dir_name
                    jack_functions.progress(
                        "all", "mkdir", jack_freedb.dir_created)

        if not cf['_multi_mode']:
            if not os.path.exists(cf['_toc_file']):
                jack_functions.cdrdao_puttoc(
                    cf['_toc_file'], jack_ripstuff.all_tracks, jack_freedb.freedb_id(jack_ripstuff.all_tracks))
                jack_freedb.freedb_template(
                    jack_ripstuff.all_tracks)  # generate freedb form if tocfile is created
...
def read_toc_file():
    ...
    if os.path.exists(cf['_toc_file']):
        ...
        jack_ripstuff.all_tracks, new_image_file, track1_offset = jack_functions.cdrdao_gettoc(
            cf['_toc_file'])

        if not os.path.exists(cf['_def_toc']):
            jack_functions.cdrdao_puttoc(
                cf['_def_toc'], jack_ripstuff.all_tracks, jack_freedb.freedb_id(jack_ripstuff.all_tracks))
    ...
    if toc_just_read and jack_helpers.helpers[cf['_ripper']].has_key("toc_cmd") and cf['_ripper'] != cf['_toc_prog']:
        ...
        if ripper_tracks != jack_ripstuff.all_tracks:
            for i in range(len(jack_ripstuff.all_tracks)):
                rtn = jack_utils.has_track(
                    ripper_tracks, jack_ripstuff.all_tracks[i][NUM])
                if rtn >= 0:
                    for j in range(6):
                        # "NUM LEN START COPY PRE CH" (not: "RIP RATE NAME")
                        if ripper_tracks[rtn][j] != jack_ripstuff.all_tracks[i][j]:
                            jack_functions.progress(i + 1, "patch", "%s %d -> %d" % (
                                fields[j], jack_ripstuff.all_tracks[i][j], ripper_tracks[rtn][j]))
                            jack_ripstuff.all_tracks[i][
                                j] = ripper_tracks[rtn][j]
                            debug("Track %02d %s" % (i + 1, fields[j]) + `jack_ripstuff.all_tracks[i][
...
def gen_todo():
    ...
    if not cf['_tracks'] and not jack_playorder.order:
        ...
        for k in tracks:
            if string.find(k, '-') >= 0:
                k = string.split(k, '-')
                lower_limit = jack_misc.safe_int(
                    k[0], "Track '%s' is not a number." % k[0])
                if k[1]:
                    upper_limit = jack_misc.safe_int(
                        k[1], "Track '%s' is not a number." % k[1])
                else:
                    upper_limit = len(jack_ripstuff.all_tracks)
                for j in range(lower_limit, upper_limit + 1):
                    tlist.append(j)
            else:
                track = jack_misc.safe_int(
                    k, "Track '%s' is not a number." % k)
                tlist.append(track)
        ...
        for k in tlist:
            if continuous:
                if k < 1 or k > len(audiotracks):
                    warning('This CD has audio tracks 1-%d.  Ignoring request for track %d.' %
                            (len(audiotracks), k))
                    continue
            else:
                if k < 1 or k > len(jack_ripstuff.all_tracks):
                    warning('This CD has tracks 1-%d.  Ignoring request for track %d.' %
                            (len(jack_ripstuff.all_tracks), k))
                    continue
...
def read_progress(status, todo):
        ...
        if status[i]['patch']:
            for j in status[i]['patch']:
                ...
                if tracknum[i][fields.index(p_what)] == p_from:
                    ...
                    error("illegal patch %s. " % j, + "Track %02d: %s is %d" %
                          (i, p_what, todo[jack_utils.has_track(todo, i)][fields.index(p_what)]))

        if status[i]['off']:
            if jack_utils.has_track(jack_ripstuff.all_tracks_todo_sorted, i) >= 0:
                del jack_ripstuff.all_tracks_todo_sorted[
                    jack_utils.has_track(jack_ripstuff.all_tracks_todo_sorted, i)]
    ...
    jack_progress.status_all['id3_genre'] = int(
        jack_progress.status_all['id3_genre'][-1])
    jack_progress.status_all['id3_year'] = int(
        jack_progress.status_all['id3_year'][-1])
...
def freedb_submit(cat=None):
    ...
    err, jack_tag.track_names, jack_tag.locale_names, cd_id, revision = jack_freedb.freedb_names(jack_freedb.freedb_id(
        jack_ripstuff.all_tracks), jack_ripstuff.all_tracks, jack_ripstuff.all_tracks, cf['_freedb_form_file'], verb=1)
    if err:
        error("invalid freedb file")
    else:
        if cf['_freedb_mailsubmit']:
            jack_freedb.do_freedb_mailsubmit(
            ...
            try:
                ...
                if pdiff:
                    ...
                    if x and x[0].upper() == "Y":
                        ...
                        freedb_submit(
                            jack_progress.status_all.get('freedb_cat', None))

    if cf['_query_on_start']:
        err, jack_tag.track_names, jack_tag.locale_names, freedb_rename, revision = jack_freedb.interpret_db_file(
            jack_ripstuff.all_tracks, todo, cf['_freedb_form_file'], verb=cf['_query_on_start'], dirs=1)
        if err:
            error(
                "query on start failed to give a good freedb file, aborting.")
    else:
        err, jack_tag.track_names, jack_tag.locale_names, freedb_rename, revision = jack_freedb.interpret_db_file(
            jack_ripstuff.all_tracks, todo, cf['_freedb_form_file'], verb=cf['_query_on_start'], warn=cf['_query_on_start'])
    ...
    for track in todo:
        ...
        if os.path.exists(mp3):
            ...
            # but to reduce confusion, the check is then removed:
                ...
                space = space + jack_utils.filesize(mp3)
                remove_q.append(mp3)
                jack_status.enc_status[track[NUM]] = "encoded file too small by " + jack_functions.pprint_i(
                    jack_functions.tracksize(track)[ENC] - jack_utils.filesize(mp3)) + "."
            elif not cf['_vbr'] and jack_utils.filesize(mp3) >= jack_functions.tracksize(track)[ENC] * 1.05:  # found by trial'n'err
                space = space + jack_utils.filesize(mp3)
                remove_q.append(mp3)
                jack_status.enc_status[track[NUM]] = "enc. file too large by " + jack_functions.pprint_i(
                    jack_utils.filesize(mp3) - jack_functions.tracksize(track)[ENC]) + "."
            else:
    ...
    for track in todo:
        ...
        if os.path.exists(wav):
            if cf['_overwrite']:
                ...
                remove_q.append(wav)
                jack_status.dae_status[
                    track[NUM]] = "Existing WAV will be overwritten."
                ...
                remove_q.append(wav)
                jack_status.dae_status[
                    track[NUM]] = " ---- [Existing WAV not done by jack.]"
                ...
                remove_q.append(wav)
                jack_status.dae_status[
                    track[NUM]] = " ---- [Existing WAV was not complete.]"
                if jack_status.enc_status[track[NUM]] == "[file lost-doing again]":
                    jack_status.enc_status[track[NUM]] = ""
        else:
            if jack_status.dae_status[track[NUM]]:
                if jack_status.enc_status[track[NUM]] == "[file lost-doing again]":
                    jack_status.dae_status[
                        track[NUM]] = " ---- [    both lost, doing again    ]"
                    jack_status.enc_status[track[NUM]] = ""
                elif cf['_keep_wavs'] or track not in jack_encstuff.mp3s_ready:
                    jack_status.dae_status[
                        track[NUM]] = " ---- [ WAV lost, doing again        ]"
...
def print_todo(todo, wavs_todo, mp3s_todo):
    ...
    for i in jack_ripstuff.all_tracks:
        ...
        if i in wavs_todo:
            print ":DAE:",
            # FIXME!
            if jack_status.dae_status[i[NUM]] != "[simulated]":
                print jack_status.dae_status[i[NUM]],
            if not cf['_only_dae']:
                print ":ENC:",
...
def check_space(space, wavs_todo, mp3s_todo):
            ...
            if space + freeable_space > jack_functions.tracksize(i)[ENC]:
                if not cf['_keep_wavs']:
                    freeable_space = freeable_space + \
                        jack_functions.tracksize(i)[
                            WAV] - jack_functions.tracksize(i)[ENC]
            else:
                will_work = 0
                # this is quite dirty
                space_needed = jack_functions.tracksize(
    ...
    if (space + freeable_space < space_needed or not will_work) and not cf['_dont_work']:
        error(("insufficient disk space (%sBytes needed), " + "try reorder or " * (not cf['_reorder']) + "free %sBytes.") % (
            jack_functions.pprint_i(space_needed - freeable_space, "%i %s"), jack_functions.pprint_i(space_needed - freeable_space - jack_ripstuff.raw_space, "%i %s")))
