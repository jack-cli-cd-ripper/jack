...
replacement_chars:[".\u200b"]""",
    ...
    'toc_mcn': {
        'type': bool,
        'val': 1,
        'usage': "read the MCN (catalog number) from the disc (old discs and CD-Rs have none, and the drive takes long to give up)",
    ...
    'toc_isrc': {
        'type': bool,
        'val': 1,
        'usage': "read the ISRCs from the disc (old discs and CD-Rs have none, and the drive takes long to give up)",
    ...
    'albumart_search': {
        'type': list,
        'val': [r'.*[Cc]over.*\.(jpg|jpeg|png)$', r'.*[Ff]ront.*\.(jpg|jpeg|png)$', r'^[Ff]older\.(jpg|jpeg|png)$', r'^jack\.caa\.front.*\.jpg$', r'^jack\.itunes.*\.jpg$', r'^jack\.discogs.*\.jpg$'],
    ...
    'albumart_ignorecase': {
        'type': bool,
        'val': 1,
        'usage': "ignore case when searching for local album art",
        'long': 'AUTO',
    },
    'albumart_recurse': {
        'type': bool,
        'val': 0,
        'usage': "recurse into subfolders when searching for local album art",
        'long': 'AUTO',
    },
    'albumart_max_size': {
        'type': int,
        'val': 1000000,
        'usage': "maximum size when considering album art file",
        'long': 'AUTO',
    },
    'albumart_min_size': {
        'type': int,
        'val': 5000,
        'usage': "minimum size when considering album art file",
        'long': 'AUTO',
    },
    'albumart_max_width': {
        'type': int,
        'val': 1200,
        'usage': "maximum width when considering album art file",
        'long': 'AUTO',
    },
    'albumart_min_width': {
        'type': int,
        'val': 250,
        'usage': "minimum width when considering album art file",
        'long': 'AUTO',
    },
    'albumart_max_height': {
        'type': int,
        'val': 1200,
        'usage': "maximum height when considering album art file",
        'long': 'AUTO',
    },
    'albumart_min_height': {
        'type': int,
    ...
    'overwrite_albumart': {
        ...
        'doc': "'conditional' downloads again when the remote length or modification time differs. Discogs re-encodes its images now and then, so for discogs an image already on disk is recognized by the image id in its name and kept; 'never' then skips discogs altogether once files for the release exist, and only 'always' downloads again.",
        'long': 'AUTO',
    },
    'albumart_providers': {
        'type': list,
    ...
    'discogs_albumart_prefix': {
        'type': str,
        'val': "jack.discogs.",
        'usage': "prefix for saving fetched discogs album art files",
        'long': 'AUTO',
    },
    'discogs_albumart_types': {
        'type': list,
        'val': ['primary'],
        'usage': "download these album arts from discogs ('primary' and/or 'secondary')",
        'long': 'AUTO',
    },
    'discogs_albumart_token': {
        ...
        'val': None,
        'secret': True,
        'usage': "discogs personal access token, optional: raises the discogs request limit from 25 to 60 per minute",
        'doc': "discogs serves its images without a token these days; the token only raises the API limit from 25 requests per minute per IP address to 60 per token. To skip discogs altogether, leave it out of albumart_providers.",
        'long': 'AUTO',
    },
