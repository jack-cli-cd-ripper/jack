...
def metadata_template(tracks, names="", revision=0):
    ...
    if api == 'cddb':
        return freedb_template(tracks, names="", revision=0)
    elif api == 'musicbrainzngs':
        return musicbrainz_template(tracks, names="", revision=0)
    else:
...
def metadata_query(cd_id, tracks, file):
    api = get_metadata_api(cf['_metadata_server'])
    if api == 'cddb':
        return freedb_query(cd_id, tracks, file)
    elif api == 'musicbrainzngs':
        return musicbrainz_query(cd_id, tracks, file)
    else:
        error("unknown api %s", api)
...
def musicbrainz_query(cd_id, tracks, file):
    ...
    try:
        ...
        print(cd_id['musicbrainzngs'], "not found or bad response")
        err = 1
        return err
...
def metadata_names(cd_id, tracks, todo, name, verb=0, warn=1):
    api = get_metadata_api(cf['_metadata_server'])
    if api == 'cddb':
        return freedb_names(cd_id, tracks, todo, name, verb=0, warn=1)
    elif api == 'musicbrainzngs':
        return musicbrainz_names(cd_id, tracks, todo, name, verb=0, warn=1)
    else:
        error("unknown api %s", api)
