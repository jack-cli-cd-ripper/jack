...
def gettoc(toc_prog):
    "Returns track list"
    if jack.helpers.helpers[toc_prog].has_key('toc_cmd'):
        cmd = string.replace(jack.helpers.helpers[
                             toc_prog]['toc_cmd'], "%d", cf['_cd_device'])
        p = os.popen(cmd)
        start = 0
        erg = []
        l = p.readline()
    ...
    for i in names:
        ...
        if i_ext == ".MP3":
            ...
            progr.append(
                [num, "dae", "  *   [          simulated           ]"])
            progr.append(
                [num, "enc", `x['bitrate']`, "[ s i m u l a t e d %3ikbit]" % (x['bitrate'] + 0.5)])
        elif i_ext == ".WAV":
            x = sndhdr.whathdr(i)
            if not x:
            ...
            if not extra_bytes == 0:
                ...
                yes = raw_input("May I strip %d bytes (= %.4fseconds) off the end? " %
            ...
            if ogg:
                ...
                progr.append(
                    [num, "dae", "  *   [          simulated           ]"])
                progr.append(
                    [num, "enc", `bitrate`, "[ s i m u l a t e d %3ikbit]" % bitrate])
            else:
        ...
        if cf['_name'] % num != i_name:
            progr.append([num, "ren", cf['_name'] %
                         num + "-->" + unicode(i_name, cf['_charset'], "replace")])
        num = num + 1
        start = start + blocks
...
# example: FILE "data.wav" 08:54:22 04:45:53
            ...
            filename = line[
                string.find(line, "\"") + 1:string.rfind(line, "\"")]
...
# by setting the pregap attribute.
            ...
            actual_track.pregap = jack.cdtime.CDTime(
                string.split(line)[1]).blocks
...
def check_genre_txt(genre):
    if isinstance(genre, int):
        ...
        if string.upper(genre) == "HELP":
            info("available genres: " + string.join(
                [x for x in eyeD3.genres if x != 'Unknown'], ", "))
            sys.exit(0)
...
def check_file(num, i, ext):
    ...
    if os.path.exists(i[NAME] + ext):
        ...
        s = jack.freedb.filenames[num].encode(
