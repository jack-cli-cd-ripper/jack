...
def freedb_query(cd_ids, tracks, file):
    ...
    url = "https://" + jack.metadata.metadata_servers[cf['_metadata_server']]['host'] + "/~cddb/cddb.cgi?" + qs
    ...
    if buf and buf[0:1] == "2":
        ...
        url = "https://" + jack.metadata.metadata_servers[cf['_metadata_server']]['host'] + "/~cddb/cddb.cgi?" + requests.compat.quote_plus(cmd + "&" + hello + "&proto=6", "=&")
        r = requests.get(url, headers=headers, stream=True)
        lines = list(r.iter_lines(decode_unicode=True))
        buf = None
        if lines:
...
def freedb_names(cd_ids, tracks, todo, name, verb=0, warn=1):
    ...
    if cf['_query_on_start'] and cf['_fetch_albumart'] and 'iTunes' in cf['_albumart_providers']:
        jack.albumart.fetch_itunes_albumart(album_artist, album_title, None)
