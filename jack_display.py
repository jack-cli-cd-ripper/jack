...
def init():
    ...
    global_total = jack_functions.tracksize(
        jack_ripstuff.all_tracks_todo_sorted)[jack_functions.BLOCKS]
    ...
    if jack_freedb.names_available:
        ...
        if jack_term.term_type == "curses":
            discname = jack_tag.locale_names[0][
                0] + " - " + jack_tag.locale_names[0][1]
        else:
            options_string = center_line(jack_tag.locale_names[0][0] + " - " + jack_tag.locale_names[0][
                                         1], fill="- ", fill_r=" -", width=jack_term.size_x) + "\n" + center_line(options_string, fill=" ", fill_r=" ", width=jack_term.size_x)
